        ...
        for row in range(grid_w):
            for col in range(grid_h):
                ...
                for b in range(nb_boxes):
                    aaa = [(x - col*cell_w)/cell_w, (y - row*cell_h)/cell_h, w/img_w, h/img_h]
                    elt += aaa + conf_t
                y_t.append(elt)
        assert(f.readline()=="---\n")
...
offset_custom = K.variable(grid)

def custom_loss(y_true, y_pred):
    global grid, offset_custom
    ...
    conf_loss_1 = K.sum(K.square((1 - y_pred_conf*iou)*y_true_conf), axis=-1)
    conf_loss_2 = K.sum(K.square((0 - y_pred_conf)*(1-y_true_conf)), axis=-1)

    # final loss function
    d = clss_loss + 3*xy_loss + 3*wh_loss + conf_loss_1 + 0.5*conf_loss_2
...
model.compile(loss=custom_loss, optimizer='rmsprop') # better
