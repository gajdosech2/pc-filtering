...
std::vector<PointFeatures> DataFormatter::GetNeighborhood(int i, int j, int tile_size)
{
  int pad = tile_size / 2;
...
void DataFormatter::GenerateDataFiles(int tile_size, int step_size)
{
  size_t height = data_.size();
  size_t width = data_[0].size();
  ...
  for (uint32_t i = outer_pad; i < height - outer_pad; i += step_size)
  {
    ...
    for (uint32_t j = outer_pad; j < width - outer_pad; j += step_size, counter++)
    {
      ...
      if (data_[i][j].intensity > EMPTY_POINT_INTENSITY)
      {
        ...
        data_file.open(DATA_FILES_ROOT + file_name_ + "_data/" + std::to_string(counter) + ".csv");
...
void DataFormatter::GenerateTruthFile(std::string truth_path, int tile_size, int step_size)
{
  int outer_pad = tile_size / 2;
