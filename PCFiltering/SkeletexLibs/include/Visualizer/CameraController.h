...
namespace vis
{
  ...
  */
  class VIS_API CameraController : public glw::WindowEventObserver
  {
  public:
    /*!
    ...
    */
    ...
    void SetScenes(const std::vector<vis::PScene> &scenes);

    //! Add scene to the list of scenes, whose internal cameras are to be controlled by mouse.
    void AddScene(const vis::PScene &scene);
    ...
    vis::CameraMode GetCameraMode(uint32_t scene_id) const;
